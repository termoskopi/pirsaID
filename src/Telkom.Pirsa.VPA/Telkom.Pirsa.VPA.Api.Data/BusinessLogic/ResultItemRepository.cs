...
{
  public class ResultItemRepository : BaseRepository, IRepository
  {
    public ResultItemRepository(IConnectionManager connection) : base(connection)
    {
      _model = new ResultItem();
      _adapter = new ResultItemAdapter(_model);
    }
    ...
    public IList<IDataModel> Get()
    {
      try
      {
        return SelectBase(new ResultItem());
      }
      catch (Exception ex)
      {
        throw ex;

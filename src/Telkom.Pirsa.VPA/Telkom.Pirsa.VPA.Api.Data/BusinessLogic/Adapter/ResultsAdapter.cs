...
{
  public class ResultsAdapter : GenericAdapter, ISqlAdapter
  {
    public ResultsAdapter(IDataModel model) : base (model)
    { 
      ...
      try
      {
        switch (queryForm)
        {
          ...
          case AdapterForm.SelectID:
            BuildSelectQuery(model, model.GetProperty(Result.ColumnId));
            break;

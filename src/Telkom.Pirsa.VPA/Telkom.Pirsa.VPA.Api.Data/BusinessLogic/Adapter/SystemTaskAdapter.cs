...
{
  public class SystemTaskAdapter : GenericAdapter, ISqlAdapter
  {
    public SystemTaskAdapter(IDataModel model) : base (model)
    { 
    ...
    public void BuildObject(System.Data.IDataReader result, bool single)
    {
      try
      {
        ...
        if (single)
        {
          if (result.Read())
          {
            IDataModel token = new SystemTask();
      ...
      try
      {
        switch (queryForm)
        {
          ...
          case AdapterForm.SelectID:
            BuildSelectQuery(model, model.GetProperty(SystemTask.ColumnId));
            break;

...
{
  public class ResultItemAdapter : GenericAdapter, ISqlAdapter
  {
    public ResultItemAdapter(IDataModel model) : base (model)
    { 
    ...
    public void BuildObject(System.Data.IDataReader result, bool single)
    {
      try
      {
        ...
        if (single)
        {
          if (result.Read())
          {
            IDataModel token = new ResultItem();
      ...
      try
      {
        switch (queryForm)
        {
          ...
          case AdapterForm.SelectID:
            BuildSelectQuery(model, model.GetProperty(ResultItem.ColumnId));
            break;

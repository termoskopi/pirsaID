...
{
  public class SystemTaskRepository : BaseRepository, IRepository
  {
    public SystemTaskRepository(IConnectionManager connection) : base(connection)
    {
      _model = new SystemTask();
      _adapter = new SystemTaskAdapter(_model);
    }
    ...
    public IList<IDataModel> Get()
    {
      try
      {
        return SelectBase(new SystemTask());
      }
      catch (Exception ex)
      {
        throw ex;
